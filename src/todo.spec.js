...
import { add, format, formatList, list, findByStatus } from "./todo.js";
...
describe("formatList", () => {
  it("should format a list of todos", () => {
    ...
    const expected = ["1 - [✅] todo title", "2 - [ ] todo title 2"];
...
describe("findByStatus", () => {
  it('should return only the done todos when "done" is passed as their status', () => {
    const mockStore = createMockStore([
    ...
    const test = findByStatus(mockStore, "done");

    expect(test).toStrictEqual(expected);
  });
  it('should return only the  unfinished todos when "not-done" is passed as their status', () => {
    const mockStore = createMockStore([
      { id: 1, title: "Todo 1", done: false },
      { id: 2, title: "Todo 2", done: true },
    ...
    const test = findByStatus(mockStore, "not-done");
